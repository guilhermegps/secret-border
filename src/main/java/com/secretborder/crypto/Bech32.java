...
public class Bech32 {
    ...
    public static String toBech32(Bech32Prefix hrp, byte[] hexKey) throws RuntimeException {
        var data = convertBits(hexKey, 8, 5, true);

        return Bech32.encode(Bech32.Encoding.BECH32, hrp.getCode(), data);
    }

    public static String toBech32(Bech32Prefix hrp, String hexKey) throws RuntimeException {
        var data = KeyUtil.hexToBytes(hexKey);

        return toBech32(hrp, data);
    }

    // Added by squirrel
    public static String fromBech32(String strBech32) throws RuntimeException {
        ...
        return KeyUtil.bytesToHex(data);
    }
     ...
     * @param bech32
     * @return
     * @throws nostr.util.RuntimeException
     */
    public static String encode(final Bech32Data bech32) throws RuntimeException {
     ...
     * @throws nostr.util.RuntimeException
     */
    // Modified to throw RuntimeExceptions
    public static String encode(Encoding encoding, String hrp, final byte[] values) throws RuntimeException {
        if (hrp.length() < 1) {
            throw new RuntimeException("Human-readable part is too short");
        }
    ...
    public static Bech32Data decode(final String str) throws RuntimeException {
        ...
        for (int i = 0; i < str.length(); ++i) {
            char c = str.charAt(i);
            if (c < 33 || c > 126) {
                throw new RuntimeException(String.format("Invalid Character %c, %d", c, i));
            }
            if (c >= 'a' && c <= 'z') {
                if (upper) {
                    throw new RuntimeException(String.format("Invalid Character %c, %d", c, i));
        ...
        if (pos < 1) {
            throw new RuntimeException("Missing human-readable part");
        }
        final int dataPartLength = str.length() - 1 - pos;
        if (dataPartLength < 6) {
            throw new RuntimeException(String.format("Data part too short: %d)", dataPartLength));
        }
        byte[] values = new byte[dataPartLength];
        for (int i = 0; i < dataPartLength; ++i) {
            char c = str.charAt(i + pos + 1);
            if (CHARSET_REV[c] == -1) {
                throw new RuntimeException(String.format("Invalid Character %c, %d", c, i + pos + 1));
            }
